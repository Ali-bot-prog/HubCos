      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-16">
          ...
          <p className="text-gray-400 mt-6 max-w-2xl mx-auto text-lg leading-relaxed">
            Endüstriyel soğutma kuleleri ve hizmetlerimiz hakkında merak
            edilenler.
          </p>
        </div>
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          {faqs.map((faq, index) => (
            <div
              key={faq.id}
              className={`bg-neutral-800 rounded-2xl border ${
                openIndex === index
                  ? "border-[#C5A059]"
                  : "border-neutral-700"
              ...
              >
                <span
                  className={`text-lg font-bold ${
                    openIndex === index ? "text-[#C5A059]" : "text-white"
                  } transition-colors`}
                >
                  {faq.question}
                </span>
                {openIndex === index ? (
                  <Minus className="w-5 h-5 text-[#C5A059] shrink-0" />
                ) : (
