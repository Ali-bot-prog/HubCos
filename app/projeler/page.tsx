        ...
        {projects.map((project, index) => (
          <FadeIn key={project.id} delay={0.1 * index}>
            <Link href={`/projeler/${project.id}`} className="block group">
              <div className="bg-neutral-900 rounded-lg shadow-sm overflow-hidden hover:shadow-xl hover:shadow-black/40 transition-all duration-300 border border-gray-800 hover:border-[#C5A059]/40 h-full flex flex-col">
                <div className="relative h-72 overflow-hidden shrink-0">
                  ...
                  <div className="absolute top-4 right-4 z-20">
                    <span className={`px-4 py-1 text-xs font-bold uppercase tracking-wider text-white ${project.status === "Tamamlandı" ? "bg-green-800" :
                ...
                <div className="p-6 flex flex-col flex-1">
                  ...
                  <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-[#C5A059] transition-colors">{project.title}</h3>
                  ...
                  <div className="mt-auto flex items-center gap-2 text-[#C5A059] text-sm font-bold uppercase tracking-wider">
                    Vaka Çalışması <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
                  </div>
                </div>
              </div>
            </Link>
