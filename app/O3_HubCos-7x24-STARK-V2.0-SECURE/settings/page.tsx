...
import { Save, ArrowLeft, Paintbrush, Layout, Type, Image as ImageIcon, Monitor, Layers, Globe } from "lucide-react";
import Link from "next/link";

const TABS = [
  { id: "general", label: "Genel", icon: Globe },
  { id: "theme", label: "Tema & Renkler", icon: Paintbrush },
  { id: "hero", label: "Hero (Giriş)", icon: Layers },
  { id: "typography", label: "Tipografi", icon: Type },
  { id: "sections", label: "Bölümler", icon: Monitor },
];
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      case "general":
        return (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-[#C5A059] mb-4">Genel Site Bilgileri</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#C5A059]"
                        value={data.siteTitle || ""} onChange={(e) => setData({...data, siteTitle: e.target.value})} />
                </div>
                <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">Footer Metni</label>
                    <input type="text" className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#C5A059]"
                        value={data.footerText || ""} onChange={(e) => setData({...data, footerText: e.target.value})} />
                </div>
                <div>
      ...
      case "theme":
        return (
          <div className="space-y-6">
            <h2 className="text-xl font-bold text-[#C5A059] mb-4">Renkler ve Düzen</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                 <div>
                ...
                <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">Görünüm Modu</label>
      ...
      case "hero":
        return (
           <div className="space-y-6">
            <h2 className="text-xl font-bold text-[#C5A059] mb-4">Hero (Giriş) Bölümü Ayarları</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <div className="flex gap-2">
                            <input type="color" className="h-12 w-12 rounded-lg cursor-pointer bg-transparent border-none p-0"
                                value={data.heroTitleColor || "#ffffff"} onChange={(e) => setData({...data, heroTitleColor: e.target.value})} />
                            <input type="text" className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#C5A059] uppercase"
                                value={data.heroTitleColor || ""} onChange={(e) => setData({...data, heroTitleColor: e.target.value})} />
                    </div>
                </div>
      ...
      case "sections":
        return (
           <div className="space-y-6">
            <h2 className="text-xl font-bold text-[#C5A059] mb-4">Bölüm Ayarları</h2>
            
            {/* About Section */}
            <div className="bg-gray-900/50 p-6 rounded-xl border border-gray-700">
                <h3 className="font-bold text-white mb-4">Hakkımızda Bölümü</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div>
                        <label className="block text-sm font-medium text-gray-400 mb-2">Arka Plan Rengi</label>
                        <div className="flex gap-2">
                             <input type="color" className="h-10 w-10 rounded-lg cursor-pointer bg-transparent border-none p-0"
                                value={data.aboutBackgroundColor || "#transparent"} onChange={(e) => setData({...data, aboutBackgroundColor: e.target.value})} />
                     ...
                     <div>
                        ...
                        <div className="flex gap-2">
                             <input type="color" className="h-10 w-10 rounded-lg cursor-pointer bg-transparent border-none p-0"
                                value={data.aboutTextColor || "#9CA3AF"} onChange={(e) => setData({...data, aboutTextColor: e.target.value})} />
                             <input type="text" className="w-full bg-gray-900 border border-gray-700 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-[#C5A059] text-sm"
                                value={data.aboutTextColor || ""} onChange={(e) => setData({...data, aboutTextColor: e.target.value})} />
                        </div>
                    </div>
                    <div className="md:col-span-2">
                        <label className="block text-sm font-medium text-gray-400 mb-2">Görsel URL</label>
                        <input type="text" className="w-full bg-gray-900 border border-gray-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-[#C5A059]"
                            value={data.aboutImage || ""} onChange={(e) => setData({...data, aboutImage: e.target.value})} 
                            placeholder="https://..." />
                    </div>
                </div>
            </div>
            ...
            {/* UI Elements */}
             <div className="bg-gray-900/50 p-6 rounded-xl border border-gray-700">
                <h3 className="font-bold text-white mb-4">UI Elementleri</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                     <div>
  ...
  return (
    <div className="max-w-6xl mx-auto pb-20">
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
            <h1 className="text-3xl font-bold">Gelişmiş Site Ayarları (CMS)</h1>
            <p className="text-gray-400">Sitenizin tüm tasarım ve içerik ayarlarını buradan yönetebilirsiniz.</p>
        </div>
      </div>
